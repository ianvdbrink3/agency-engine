...
function ClusterCard({ cluster }: { cluster: KeywordCluster }) {
  const totalVolume = cluster.totalVolume ?? 0;
  const avgDifficulty = cluster.avgDifficulty ?? 0;
    ...
    <Card className="border border-border/60">
      <CardHeader className="pb-3 pt-4 px-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <CardTitle className="text-sm font-semibold">{cluster.name || "Naamloos cluster"}</CardTitle>
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground mb-3">
          <span className="tabular-nums">
            <span className="font-semibold text-foreground">
              {totalVolume.toLocaleString("nl-NL")}
            </span>{" "}
            vol/mnd
          </span>
          <span className="tabular-nums">
            <span className="font-semibold text-foreground">{avgDifficulty}</span> gem. KD
          </span>
          <span>
            <span className="font-semibold text-foreground">{keywords.length}</span> zoekwoorden
          </span>
        </div>
...
function PillarPageCard({ page }: { page: PillarPage }) {
  const clusterPages = Array.isArray(page.clusterPages) ? page.clusterPages : [];
  const totalVolume = page.totalVolume ?? 0;
      ...
      <CardContent className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold truncate">{page.title || "Naamloos"}</p>
            {page.slug && (
              <p className="text-xs text-muted-foreground font-mono mt-0.5 truncate">/{page.slug}</p>
            )}
            <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
              <span className="tabular-nums">
                <span className="font-semibold text-foreground">{totalVolume.toLocaleString("nl-NL")}</span> vol
              </span>
              <span>
                <span className="font-semibold text-foreground">{clusterPages.length}</span> cluster pagina's
              </span>
            </div>
            {clusterPages.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {clusterPages.slice(0, 3).map((cp, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className="text-xs bg-muted/40 border-border text-muted-foreground"
                  >
                    {typeof cp === "string" ? cp : cp.title}
                  </Badge>
                ))}
